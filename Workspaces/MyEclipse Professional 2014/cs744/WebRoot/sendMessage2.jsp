...
<%}
		...
		<%
  		if(edge != null) { // 有信息返
			...
			edges.push({from :<%=edgeNN.get(i).getNode1()%>, to :<%=edgeNN.get(i).getNode2()%>,smooth: {type: 'dynamic'},length : EDGE_LENGTH_SUB});
			<%}}
  		}%>
  		<%
  		if(pathEdge != null) { // 有信息返
  			...
  			%>		
			edges.push({from :<%=pathEdge.get(i).getNode1()%>, to :<%=pathEdge.get(i).getNode2()%>,arrows:'to',color:'#FFA807',length : EDGE_LENGTH_MAIN,dashes:true});
			<%} else { %>
			edges.push({from :<%=pathEdge.get(i).getNode1()%>, to :<%=pathEdge.get(i).getNode2()%>,arrows:'to',color:'#FFA807'});
  		<%}}
  		}%>
	// create a network
